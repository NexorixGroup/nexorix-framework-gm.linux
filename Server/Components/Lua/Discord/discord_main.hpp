...
struct DiscordOutMsg {
    ...
    std::string content;    // texto simples (vazio se for embed)
    std::string embed_json; // JSON do embed (vazio se for mensagem simples)
};
...
// ---------------------------------------------------------------------------
// DiscordComponent — embedded inside Lua.dll (not a standalone component)
// Does NOT inherit IComponent — it's a plain class managed by LuaComponent.
// ---------------------------------------------------------------------------
...
public:
    ...
    // Called by LuaComponent lifecycle
    void onLoad(ICore* core);
    void onInit(IComponentList* components);
    void onReady();

    // Called every server tick (main thread) — flushes inbound event queue to Lua
    void tick(lua_State* L);

    // Lua API surface
    void sendMessage(const std::string& channel_id, const std::string& content);
    void sendEmbed(const std::string& channel_id, const std::string& embed_json);
...
private:
    ...
    void workerLoop();
    void gatewayLoop(void* ssl);   // SSL* passado como void* para evitar include OpenSSL no header
    std::string fetchGatewayUrl();
    void* gatewayConnect(const std::string& gateway_url); // retorna SSL*
